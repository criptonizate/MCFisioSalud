...
import { Calendar, Clock, RefreshCw, MessageCircle, CreditCard, ArrowRight } from 'lucide-react'
...
function TurnoCard({ t, config, onCancel, onReagendar }) {
  const fd = t.fecha?.toDate ? t.fecha.toDate() : null
  const relLabel = getRelativeLabel(t.fecha)
  const canc = ['pendiente', 'confirmado'].includes(t.estado) && canCancel(t, config?.horasLimiteCancelacion)
  ...
  return (
    <div className={`bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex ${t.estado === 'cancelado' ? 'opacity-55' : ''}`}>
      ...
      <div className="flex-1 p-4 space-y-2.5">
        ...
        {t.estado === 'pendiente' && (
          <p className="text-xs text-yellow-700 bg-yellow-50 border border-yellow-100 rounded-lg px-3 py-1.5">
            Pendiente de confirmación por el kinesiólogo. Revisá esta pantalla en unas horas.
          </p>
        )}
        ...
        {['pendiente', 'confirmado'].includes(t.estado) && (
          <div className="flex flex-wrap items-center gap-3 pt-0.5">
            {canc ? (
              <button onClick={() => onCancel(t)} className="text-xs text-red-500 hover:text-red-700 hover:underline">
                Cancelar turno
              </button>
            ) : (
              <p className="text-xs text-gray-400">Cancelación no disponible (menos de {config?.horasLimiteCancelacion || 24}hs)</p>
            )}
            <button onClick={() => onReagendar(t)} className="text-xs text-[#1565C0] hover:underline flex items-center gap-1">
              <RefreshCw className="w-3 h-3" /> Cambiar turno
            </button>
          </div>
        )}
      </div>
...
export default function MisTurnos() {
  ...
  const [reagendarTurno, setReagendarTurno] = useState(null)
  ...
  async function handleReagendar() {
    setReagendando(true)
    try {
      await actualizarTurno(reagendarTurno.id, { estado: 'cancelado', canceladoPor: 'paciente' })
      toast({ message: 'Turno cancelado. Elegí uno nuevo.', type: 'info' })
  ...
  return (
    <div className="space-y-6">
                ...
                <div key={t.id} className="bg-white rounded-xl border-2 border-purple-200 overflow-hidden flex">
                  ...
                  <div className="flex-1 p-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => responderPropuesta(t, true)} loading={respondiendo} className="flex-1">Aceptar</Button>
                      <Button size="sm" variant="secondary" onClick={() => responderPropuesta(t, false)} loading={respondiendo} className="flex-1">Rechazar</Button>
                    </div>
                  </div>
                </div>
              )
      ...
      {pasados.length > 0 && (
        <section>
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">
            Historial <span className="ml-1 normal-case font-normal">({pasados.length} sesiones)</span>
          </h2>
          <div className="space-y-3">
            {pasados.slice(0, historialLimit).map(t => (
              <TurnoCard key={t.id} t={t} config={config} onCancel={() => {}} onReagendar={() => {}} />
            ))}
          </div>
          {pasados.length > historialLimit && (
            <button
              onClick={() => setHistorialLimit(n => n + HISTORIAL_PAGE)}
              className="mt-3 w-full flex items-center justify-center gap-2 text-sm text-[#1565C0] hover:underline py-2"
            >
              <ArrowRight className="w-4 h-4" />
              Ver {Math.min(HISTORIAL_PAGE, pasados.length - historialLimit)} más ({pasados.length - historialLimit} restantes)
            </button>
          )}
        </section>
      )}
      ...
      {/* Modal reagendar */}
      <Modal open={!!reagendarTurno} onClose={() => setReagendarTurno(null)} title="Cambiar turno" size="sm">
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
            <p className="font-medium mb-1">¿Querés elegir otro horario?</p>
            <p>Se cancelará tu turno del <strong>{reagendarTurno?.fecha ? getRelativeLabel(reagendarTurno.fecha) : ''}</strong> a las <strong>{reagendarTurno?.horaInicio}hs</strong> y podrás reservar uno nuevo.</p>
          </div>
          <div className="flex gap-2">
            <Button loading={reagendando} onClick={handleReagendar} className="flex-1">Sí, cambiar turno</Button>
            <Button variant="secondary" onClick={() => setReagendarTurno(null)} className="flex-1">Cancelar</Button>
          </div>
        </div>
      </Modal>
    </div>
